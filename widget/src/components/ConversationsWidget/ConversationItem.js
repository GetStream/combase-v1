...
const ConversationItem = ({ active, id, statusBorder, theme }) => {
				...
				<Root>
					<Wrapper {...{ active }}>
						<Avatar
							...
							size={48}
							showStatus={true} // TODO: This should be true/false based on org wide availability times.
							{...{ statusBorder }}
						/>
						<Content>
							<Row>
								<Text size={12} faded weight="500">Connecting you to an agent...</Text>
							</Row>
							<Row>
								<Fill />
								<Chip label="PENDING" color="gray" size={8} />
							</Row>
						</Content>
					</Wrapper>
				</Root>
	...
	return <Route path={`/inbox/${id}`} children={partner ? renderItem : renderWaiting} />;
};

export default withTheme(ConversationItem);
